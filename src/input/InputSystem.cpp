...
{
    //("[DEBUG_LOAD] LoadBindings() called with path: %s\n", path.c_str());
    ...
    if (!file.is_open())
    {
        //("[ERROR] Failed to open bindings file: %s\n", path.c_str());
        std::cerr << "[Input] Failed to open bindings file: " << path << '\n';
        return;
    }
    //("[DEBUG_LOAD] File opened successfully\n");

    json data;
    try
    {
        file >> data;
        //("[DEBUG_LOAD] JSON parsed successfully\n");
    }
    catch (const std::exception& e)
    {
        //("[ERROR] Failed to parse JSON: %s\n", e.what());
    ...
    if (auto mouseIt = data.find("mouse"); mouseIt != data.end() && mouseIt->is_object())
    {
        ...
        //("[DEBUG_LOAD] Mouse settings: sensitivity=%.2f, smoothtype=%s\n", m_mouseSettings.sensitivity, smooth.c_str());
        if (smooth == "EMA")
        {
    ...
    {
        //("[DEBUG_LOAD] No 'mouse' section found in JSON\n");
    ...
    if (auto axesIt = data.find("axes"); axesIt != data.end() && axesIt->is_object())
    {
        ...
        for (auto& [axisName, bindings] : axesIt->items())
        {
            ...
            //("[DEBUG_LOAD] Axis '%s' loaded with %zu bindings\n", axisName.c_str(), entry.bindings.size());
        }
        //("[DEBUG_LOAD] Total axes loaded: %zu\n", m_axes.size());
    }
    else
    {
        //("[ERROR] No 'axes' section found in JSON\n");
    }

    if (auto actionsIt = data.find("actions"); actionsIt != data.end() && actionsIt->is_object())
    {
        //("[DEBUG_LOAD] Found 'actions' section\n");
        for (auto& [actionName, bindings] : actionsIt->items())
        {
            //("[DEBUG_LOAD] Loading action '%s'\n", actionName.c_str());
            ActionEntry entry;
            if (bindings.is_array())
            {
                for (const auto& binding : bindings)
        ...
        //("[DEBUG_LOAD] Total actions loaded: %zu\n", m_actions.size());
    }
    else
    {
        //("[ERROR] No 'actions' section found in JSON\n");
    }
    ...
    if (!ec)
    {
        ...
        //("[DEBUG_LOAD] File modification time tracked\n");
    }
    else
    {
        m_hasLastWriteTime = false;
        //("[DEBUG_LOAD] Could not track file modification time\n");
    }
    
    //("[DEBUG_LOAD] LoadBindings() complete. Axes: %zu, Actions: %zu\n", m_axes.size(), m_actions.size());
}
...
void InputSystem::UpdateAxes()
{
    ...
    if (m_mouseSettings.smoothing == MouseSettings::SmoothType::Ema)
    {
        if (!m_mouseInitialized)
        {
            ...
            //("[DEBUG_INPUT] Mouse smoothing initialized (EMA)\n");
        }
        else
        {
    ...
    //("[DEBUG_INPUT] Processing %zu axes\n", m_axes.size());
    
    for (auto& [axisName, entry] : m_axes)
    {
        float value = 0.0f;
        //("[DEBUG_INPUT] Axis '%s': %zu bindings\n", axisName.c_str(), entry.bindings.size());
        ...
        for (const auto& binding : entry.bindings)
        {
            ...
            case AxisBinding::Type::Key:
                ...
                {
                    value += binding.scale;
                    //("[DEBUG_INPUT]   Key binding: key=%d, scale=%.2f, NEW_VALUE=%.2f\n", binding.key, binding.scale, value);
                }
                break;
            case AxisBinding::Type::MouseDelta:
                if (binding.mouseAxis == AxisBinding::MouseAxis::DeltaX)
                {
                    ...
                    //("[DEBUG_INPUT]   Mouse DeltaX binding: scaledDx=%.2f, scale=%.2f, NEW_VALUE=%.2f\n", scaledDx, binding.scale, value);
                }
                else
                {
        ...
        //("[DEBUG_INPUT] Axis '%s' final value: %.2f\n", axisName.c_str(), entry.value);
    }
}